...
class AGOSAnalyticsSystem {
  ...
  updateRealTimeDisplays() {
    ...
    const currentLevel = this.elements["current-level"];
    const currentFlow = this.elements["current-flow"];

    if (currentLevel && this.state.currentData) {
      currentLevel.textContent = `${this.state.currentData.waterLevel.toFixed(
        2
      )} cm`;
    }

    if (currentFlow && this.state.currentData) {
      currentFlow.textContent = `${this.state.currentData.flowRate.toFixed(
        2
      )} m/s`;
    }
  }
  ...
  updateSystemStatus(status) {
    ...
    const dataStatus = this.elements["data-status"];
    const analysisStatus = this.elements["analysis-status"];

    if (dataStatus) {
      dataStatus.textContent = this.state.dataStatus.toUpperCase();
    }

    if (analysisStatus) {
      analysisStatus.textContent = this.state.analysisStatus.toUpperCase();
    }
  }
  ...
  showLoading(message) {
    ...
    const loadingText = this.elements["loading-text"];
