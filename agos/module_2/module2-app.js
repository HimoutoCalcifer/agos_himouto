...
class AGOSMappingSystem {
  ...
  displayAnalysisResults(results) {
    ...
    // Update timestamp
    const timestampElement = this.elements["analysis-timestamp"];
    if (timestampElement) {
      timestampElement.textContent = results.timestamp.toLocaleString();
    }
  }
  ...
  updateTimeDisplays() {
    ...
    if (lastAnalysisElement && this.state.lastAnalysis) {
      const now = new Date();
      const diff = Math.floor((now - this.state.lastAnalysis) / 1000);
      lastAnalysisElement.textContent =
        diff < 60
          ? `${diff}s ago`
          : diff < 3600
